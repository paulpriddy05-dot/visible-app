    ...
    <div className="pb-20 min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 transition-colors duration-300">
      <nav className="bg-slate-900 text-white sticky top-0 z-40 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:items-center sm:justify-between sm:flex-row gap-3 sm:gap-0 py-3 sm:py-0 sm:h-14">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-lg tracking-tight text-white truncate max-w-[150px] sm:max-w-none">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="pl-8 pr-4 py-1.5 bg-slate-800 border border-slate-700 rounded-full text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 w-full sm:w-48 md:w-64 transition-all"
                />
              </div>
              {canEdit && (
                <button
          ...
          {scheduleCards.length > 0 && (
            <div className="space-y-4">
              ...
              <SortableContext items={weeklyScheduleItems} strategy={rectSortingStrategy} id="Weekly Schedule">
                <div className="grid grid-cols-1 gap-4 min-h-[100px] border-2 border-transparent hover:border-slate-200/50 border-dashed rounded-xl transition-all sm:grid-cols-2 lg:grid-cols-3">
            ...
            return (
              <div key={section} className="space-y-4 animate-in fade-in duration-500">
                ...
                <SortableContext items={sectionCards} strategy={rectSortingStrategy} id={section}>
                  <div className="grid grid-cols-1 gap-4 min-h-[100px] rounded-xl border-2 border-dashed border-transparent p-2 transition-colors hover:border-slate-200/20 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {/* --- MODAL --- */}
      {activeCard && (
        <div onClick={() => setActiveModal(null)} className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-0 sm:px-4 overflow-hidden bg-slate-900/70 backdrop-blur-sm">
          <div onClick={(e) => e.stopPropagation()} className={`bg-white w-full max-w-[100vw] sm:max-w-2xl md:max-w-6xl rounded-t-2xl sm:rounded-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] sm:h-auto sm:max-h-[85vh]`}>
            {/* Header */}
            <div className={`${getBgColor(activeCard.color || 'rose')} p-4 sm:p-6 flex justify-between items-center text-white shrink-0 transition-colors`}>
              <div>
                <h3
                  ref={titleRef}
                  contentEditable={isEditing && canEdit}
                  suppressContentEditableWarning={true}
                  className={`text-xl sm:text-2xl font-bold outline-none ${isEditing && canEdit ? 'border-b-2 border-white/50 bg-white/10 px-2 rounded cursor-text' : ''} line-clamp-1`}
                >
                  {activeCard.title}
                </h3>
                {isEditing && isCardEditable(activeCard) && canEdit && (
                  <div className="flex gap-2 mt-3 animate-in fade-in slide-in-from-left-2 duration-200 overflow-x-auto pb-2">
              ...
              <div className="flex items-center gap-1 sm:gap-3 flex-wrap">
                ...
                {(activeCard.type === 'generic-sheet' || attachedSheetUrl || activeCard.data) && !isMapping && (
                  <button
                    ...
                    className={`px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold transition-all flex items-center gap-1 sm:gap-2 shadow-sm border rounded-lg ${activeCard.settings?.viewMode
                  ...
                  >
                    ...
                    {activeCard.settings?.viewMode ? "Files" : "Data"}
                  </button>
                )}
                {activeCard.settings?.viewMode && (
                  <button
                    onClick={() => setIsMapping(true)}
                    className={`px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 transition-all shadow-sm rounded-lg ${!canEdit ? 'hidden' : ''}`}
                ...
                {(showDocPreview || activeCard.sheet_url || activeCard.source?.includes("sheet")) && (
                  <a href={showDocPreview ? showDocPreview.replace("/preview", "/edit") : activeCard.sheet_url || activeCard.sheet_url_schedule} target="_blank" rel="noreferrer" className="px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors border border-blue-200 flex items-center gap-1 sm:gap-2 rounded-lg">
                    <i className="fas fa-external-link-alt"></i> <span className="hidden sm:inline">Open in {activeCard.source?.includes("sheet") || activeCard.type === 'generic-sheet' ? "Sheets" : "Docs"}</span>
                  </a>
                )}
                {!showDocPreview && isCardEditable(activeCard) && canEdit && (<button onClick={toggleEditMode} className={`px-2 sm:px-3 py-1 text-xs sm:text-sm font-medium transition-colors border rounded-lg ${isEditing ? 'bg-white text-slate-900 border-white' : 'bg-black/20 text-white border-transparent hover:bg-black/40'}`}><i className={`fas ${isEditing ? 'fa-check' : 'fa-pen'} mr-1 sm:mr-2`}></i>{isEditing ? "Done" : "Edit"}</button>)}
                {!showDocPreview && (isCardEditable(activeCard) || activeCard.type === 'generic-sheet') && canEdit && (
                  <button
                    onClick={() => deleteCard(activeCard)}
                    className="bg-red-500 px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold hover:bg-red-600 transition-colors text-white rounded-lg"
                    title="Delete Card"
                  >
                    <i className="fas fa-trash"></i>
                  </button>
                )}
                {showDocPreview && <button onClick={() => setShowDocPreview(null)} className="bg-white/20 px-2 sm:px-3 py-1 text-xs sm:text-sm font-medium rounded-lg"><i className="fas fa-arrow-left mr-1 sm:mr-2"></i> Back</button>}
              ...
              {activeCard.settings?.viewMode ? (
                ...
                (!activeCard.data && activeCard.type !== 'generic-sheet') ? (
                  ...
                  <div className="flex flex-col md:flex-row h-full">
                    ...
                    <div className="w-full md:w-1/3 bg-white border-b md:border-r md:border-b-0 border-slate-200 flex flex-col order-2 md:order-1">
                      ...
                      <div className="p-4 sm:p-6 border-t border-slate-200 bg-slate-50 flex gap-3 md:gap-0 flex-col md:flex-row">
                        <button
                          onClick={() => saveMapping(activeCard.settings)}
                          className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold transition-all shadow-sm flex items-center justify-center gap-2"
                        >
                          <i className="fas fa-check"></i> Save
                        </button>
                        <button onClick={() => setIsMapping(false)} className="flex-1 text-slate-500 text-sm font-medium hover:text-slate-800">Cancel</button>
                      </div>
                    </div>
                    {/* RIGHT SIDE: LIVE PREVIEW */}
                    <div className="flex-1 bg-slate-100/50 flex flex-col order-1 md:order-2">
                      ...
                      <div className="flex-1 p-4 sm:p-10 flex items-center justify-center overflow-y-auto">
                        {/* 🟢 CHART RENDER LOGIC */}
                        {activeCard.settings?.viewMode === 'chart' ? (
                          <div className="w-full h-full max-h-[50vh] sm:h-96 bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-xl flex flex-col">
                          ...
                          <div className="p-8 grid grid-cols-1 gap-6 overflow-y-auto sm:grid-cols-2 md:grid-cols-3">
                          ...
                          <div className="flex flex-col h-full bg-slate-50">
                            <div className="px-4 sm:px-8 py-4 border-b border-slate-200 bg-white flex justify-between items-center"><div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Data Source: Google Sheet</div><div className="text-xs bg-slate-100 text-slate-500 px-3 py-1 rounded-full font-mono">{activeCard.rowCount} Rows</div></div>
                            <div className="flex-1 overflow-auto p-4 sm:p-8 custom-scroll">
                              <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-x-auto">
                                <table className="min-w-full divide-y divide-slate-200">
                                  <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm"><tr>{activeCard.columns?.map((col: string, idx: number) => (<th key={idx} className="px-4 sm:px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap bg-slate-50">{col}</th>))}</tr></thead>
                                  <tbody className="bg-white divide-y divide-slate-200">{activeCard.data.map((row: any, rIdx: number) => (<tr key={rIdx} className="hover:bg-blue-50/50 transition-colors group">{activeCard.columns?.map((col: string, cIdx: number) => (<td key={cIdx} className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm">{renderCellContent(row[col])}</td>))}</tr>))}</tbody>
                                </table>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="p-4 sm:p-8 grid grid-cols-1 gap-6 overflow-y-auto sm:grid-cols-2 md:grid-cols-3">
                    {activeCard.data.filter((row: any) => row[activeCard.settings.titleCol]).map((row: any, idx: number) => (
                      <div key={idx} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                ...
                <div className="p-0 bg-slate-50 min-h-full">
                  {/* ... Missions Content ... */}
                  <div className="bg-sky-50 p-4 sm:p-8 border-b border-sky-100 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div><div className="text-xs font-bold text-sky-500 uppercase tracking-widest mb-1">Upcoming Departure</div><h2 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-1">{activeCard.upcomingLoc} Team</h2><div className="flex items-center gap-2 text-slate-500"><i className="far fa-calendar-alt"></i> {activeCard.upcomingDate}</div></div>
                    <div className="bg-white px-4 sm:px-6 py-4 rounded-xl shadow-sm border border-sky-100 text-center min-w-[120px]"><div className="text-3xl sm:text-4xl font-bold text-sky-600">{activeCard.upcomingOpen}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Open Spots</div></div>
                  </div>
                  <div className="p-4 sm:p-8"><div className="flex items-center gap-2 mb-4"><i className="fas fa-ticket-alt text-slate-400"></i><h4 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Open Registrations</h4></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{activeCard.trips.map((trip: any, idx: number) => (<div key={idx} className="bg-white p-5 rounded-xl border border-slate-200 flex justify-between items-center shadow-sm"><div><div className="font-bold text-slate-800 text-lg">{trip.name}</div><div className="text-xs text-slate-400 mt-1">Registration Open</div></div><div className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold">{trip.spots} Spots</div></div>))}</div></div>
                  <div className="bg-white p-4 sm:p-8 border-t border-slate-200"><h4 className="text-sm font-bold text-slate-400 uppercase tracking-wide mb-6">2026 Statistics</h4><div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-12"><div><div className="flex items-end gap-3 mb-2"><span className="text-3xl sm:text-4xl font-bold text-slate-800">{activeCard.totalNonStaff}</span><span className="text-sm text-slate-500 mb-1">Non-Staff ({activeCard.percentNonStaff})</span></div><div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-cyan-500 h-2 rounded-full" style={{ width: activeCard.percentNonStaff }}></div></div></div><div><div className="flex items-end gap-3 mb-2"><span className="text-3xl sm:text-4xl font-bold text-slate-800">{activeCard.totalStaff}</span><span className="text-sm text-slate-500 mb-1">Staff ({activeCard.percentStaff})</span></div><div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-purple-500 h-2 rounded-full" style={{ width: activeCard.percentStaff }}></div></div></div></div></div>
                </div>
                            ...
                            {block.items.map((item: any, iIdx: number) => (
                              <div key={iIdx} className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md hover:border-blue-400 transition-all group cursor-pointer relative">
                                <div className="flex items-center gap-4 flex-1" onClick={() => handleFileClick(item)}>
                                  ...
                                  <div className="font-bold text-slate-800 text-sm group-hover:text-blue-600 truncate">{item.title}</div>
                                </div>
                          ...
                          {isEditing && canEdit && (
                            <div className="mt-4 pt-4 border-t border-slate-50">
                              {addingLinkToBlock === bIdx ? (
                                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full animate-in fade-in slide-in-from-left-2 duration-200">
                                  <input ref={(el) => { newItemTitleRefs.current[`block-${bIdx}`] = el }} type="text" placeholder="Name your file..." autoFocus className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition-all" />
                                  <input ref={(el) => { newItemUrlRefs.current[`block-${bIdx}`] = el }} type="text" placeholder="Paste URL..." className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition-all" />
                                  <button onClick={() => addItemToBlock(bIdx)} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 transition-colors shadow-sm w-full sm:w-auto">Add</button>
                                  <button onClick={() => setAddingLinkToBlock(null)} className="text-slate-400 hover:text-slate-600 px-2 transition-colors w-full sm:w-auto"><i className="fas fa-times"></i></button>
                                </div>
                              ) : (
                                <div className="flex flex-col sm:flex-row gap-3">
                                  <button onClick={() => handleOpenPicker(bIdx)} className="px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-colors bg-slate-100 text-slate-700 hover:bg-slate-200 w-full sm:w-auto"><svg viewBox="0 0 87.3 78" width="14" height="14" className="opacity-80"><path d="M6.6 66.85l3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3l18.25-31.6H6.6v21.65zm13.1-22.8L6.6 19.85l-3.3 5.7C2.1 27.9 1.5 30.2 1.5 32.55v15.85l18.2-4.35zM22.65 6.6L19.35.9C18 .1 16.55-.3 15-.3h-3.4l18.25 31.65 14.2-24.6h-21.4zM62.6 6.6h-14.8l-7.1 12.35L54.1 36.3l14.8-25.6c-.65-1.9-1.9-3.35-3.65-4.1zM34.7 44.05l-7.1 12.3 10.75 18.6c2.35 0 4.7-.6 6.85-1.8L84.65 44.05H34.7zM66.45 44.05l18.2 31.6c1.35-.8 2.5-1.9 3.3-3.3l3.85-6.65-18.25-31.6-7.1 9.95z" fill="currentColor" /></svg>Add from Drive</button>
                                  <button onClick={() => setAddingLinkToBlock(bIdx)} className="px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-colors bg-blue-50 text-blue-700 hover:bg-blue-100 w-full sm:w-auto">+ Add Link</button>
                                </div>
                              )}
                            </div>
                          )}
      ...
      {showTutorial && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center px-4 bg-slate-900/80 backdrop-blur-sm transition-opacity">
          <div className="bg-white w-full max-w-2xl rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[90vh] sm:max-h-auto overflow-y-auto">
            {/* Header */}
            <div className="bg-slate-50 p-4 sm:p-6 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center gap-3">
            ...
            {/* Body */}
            <div className="p-4 sm:p-8 space-y-8 text-slate-600">
            ...
            <div className="bg-slate-50 p-4 sm:p-6 border-t border-slate-200 text-center">
              <button onClick={() => setShowTutorial(false)} className="bg-slate-800 hover:bg-slate-900 text-white font-bold py-3 px-10 rounded-xl transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 active:translate-y-0 w-full sm:w-auto">Got it, let's go!</button>
            </div>
          </div>
        </div>
      )}
