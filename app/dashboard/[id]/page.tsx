...
function SortableCard({ card, onClick, getBgColor, variant = 'vertical' }: any) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} onClick={() => onClick(card)} className={`cursor-grab active:cursor-grabbing hover:-translate-y-1 hover:shadow-lg rounded-2xl p-4 md:p-6 text-white flex flex-col items-center justify-center text-center h-40 relative overflow-hidden group ${getBgColor(card.color || 'rose')}`}>
      {showMiniChart ? (
        ...
        <>
          <div className="bg-white/16 p-1 rounded-full mb-3 md:mb-4 backdrop-blur-sm"><i className={`fas ${displayIcon} text-2xl md:text-3xl`}></i></div>
...
export default function DynamicDashboard() {
  ...
  useEffect(() => {
    ...
    const initDashboard = async () => {
      ...
      const { data: dashConfig, error } = await supabase.from('dashboards').select('*').eq('id', dashboardId).single();

      if (error || !dashConfig) { console.error("Dashboard Load Error:", error); return; }

      let userCanEdit = false;
      if (user) {
        const { data: accessList } = await supabase.from('dashboard_access').select('role, user_id, user_email').eq('dashboard_id', dashboardId);
        if (accessList) {
          const myPermission = accessList.find(row => row.user_id === user.id || (user.email && row.user_email?.toLowerCase() === user.email.toLowerCase()));
          if (myPermission && ['owner', 'editor', 'edit'].includes(myPermission.role?.toLowerCase())) { userCanEdit = true; }
        }
      }
      ...
      // 🟢 UPDATED: Load manual cards AND fetch data for dashboards
      ...
      if (manualData) {
        ...
        // 🚀 HYDRATION: Fetch CSV data for any card that needs to show a chart
        cardsWithSource.forEach((c: any) => {
          if (c.settings?.showOnDashboard && c.settings?.connectedSheet) {
            fetch(toCSVUrl(c.settings.connectedSheet))
              .then(r => r.text())
              .then(text => {
                Papa.parse(text, {
                  header: true, skipEmptyLines: true, transformHeader: (h: string) => h.trim(),
                  complete: (res: any) => {
                    setManualCards(prev => prev.map(p => p.id === c.id ? { ...p, data: res.data, rowCount: res.data.length, columns: res.meta.fields } : p));
                  }
                })
              })
              .catch(err => console.error("Auto-fetch failed for", c.title, err));
          }
        });
      }
  ...
  const loadSheetData = async (url: string, card: any) => {
    try {
      ...
      if (!response.ok) { console.warn(`Could not load sheet: ${response.status}`); return; }
        ...
        if (response.ok) {
          ...
          Papa.parse(csvText, {
            ...
            complete: (results: any) => {
              ...
              loadedWidgets.push({ id: widget.id, title: widget.title, type: 'generic-sheet', color: widget.color || 'blue', data: results.data, columns: results.meta.fields, rowCount: results.data.length, sheet_url: widget.sheet_url, settings: safeSettings });
            }
          });
        }
  ...
  const fetchManualCards = async () => {
    // 🟢 UPDATED: This function is now mostly redundant as initDashboard handles it, 
    // but kept for safety if called elsewhere.
  ...
  const saveMapping = async (newSettings: any) => {
    ...
    // We don't save data to DB to keep it light, but we update local state
  ...
  const updateCardCategory = async (card: any, newCategory: string) => {
    ...
    if (card.type === 'generic-sheet') { setGenericWidgets(prev => prev.map(w => w.id === card.id ? updatedCard : w)); } else { setManualCards(prev => prev.map(c => c.id === card.id ? updatedCard : c)); }
    const type = card.type === 'generic-sheet' ? 'widget' : 'manual';
    const { error } = await supabase.rpc('move_dashboard_card', { p_card_id: card.id, p_new_category: newCategory, p_card_type: type });
    ...
    openPicker({
      ...
      callbackFunction: (data: any) => {
        if (data.action === "picked") {
          if (data.oauthToken) { setGoogleToken(data.oauthToken); }
  ...
  const addFilesToBlock = async (bIdx: number, files: any[]) => {
    ...
    const newItems = files.map(file => ({ title: file.name, url: file.url, type: 'google-drive', iconUrl: file.iconUrl, mimeType: file.mimeType, fileId: file.id }));
    ...
    if (hasSheet) {
      ...
      if (confirmChart) {
        const updatedCard = { ...activeCard, resources: currentBlocks, sheet_url: hasSheet.url, settings: { ...activeCard.settings, viewMode: 'table', connectedSheet: hasSheet.url } };
        setActiveCard(updatedCard);
        setManualCards(prev => prev.map(c => c.id === activeCard.id ? updatedCard : c));
        await supabase.from('Weeks').update({ settings: updatedCard.settings, sheet_url: hasSheet.url }).eq('id', activeCard.id);
  ...
  const setActiveModal = (card: any) => {
    ...
    // Always fetch fresh data if connected
  ...
  const handleFileClick = (item: any) => {
    ...
    if (isGoogleDoc(item.url)) { setShowDocPreview(getEmbedUrl(item.url)); } else { window.open(item.url, '_blank'); }
  };
  ...
  return (
    <div className="pb-20 min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 transition-colors duration-300">
          ...
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-4 md:gap-6">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-8 pr-4 py-1.5 bg-slate-800 border border-slate-700 rounded-full text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 w-32 md:w-64 transition-all" />
              </div>
              {canEdit && (<button onClick={() => addNewCard(sections[0])} className="flex items-center gap-2 px-3 py-1.5 text-xs font-bold rounded-md bg-blue-600 border border-blue-500 hover:bg-blue-500 text-white transition-all shadow-sm hover:shadow-blue-500/20"><i className="fas fa-plus"></i><span className="hidden md:inline">New Card</span></button>)}
            </div>
            <div className="flex items-center gap-3">
              <button onClick={() => setShowInviteModal(true)} className="flex items-center gap-2 px-3 py-1.5 text-xs font-bold rounded-md bg-purple-600 border border-purple-500 hover:bg-purple-500 text-white transition-colors shadow-sm"><i className="fas fa-user-plus"></i><span className="hidden md:inline">Invite</span></button>
              <button onClick={() => setShowTutorial(true)} className="h-8 w-8 rounded-full bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors flex items-center justify-center border border-slate-700" title="Help & Tutorial"><i className="fas fa-question text-sm"></i></button>
              <button onClick={() => window.location.href = '/account'} className="h-8 w-8 rounded-full bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors flex items-center justify-center border border-slate-700 hidden md:flex" title="Account Settings"><i className="fas fa-user-cog text-sm"></i></button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
        <DndContext sensors={canEdit ? sensors : []} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          {scheduleCards.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between group">
                <div className={`flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-wider pl-1 transition-colors ${canEdit ? "cursor-pointer hover:text-blue-500" : "cursor-default"}`} onClick={() => canEdit && renameSection(scheduleTitle, 'schedule')}><i className="fas fa-calendar-alt"></i> {scheduleTitle}{canEdit && <i className="fas fa-pen opacity-0 group-hover:opacity-100 ml-2"></i>}</div>
              ...
              <SortableContext items={weeklyScheduleItems} strategy={rectSortingStrategy} id="Weekly Schedule"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 min-h-[100px] border-2 border-transparent hover:border-slate-200/50 border-dashed rounded-xl transition-all">{weeklyScheduleItems.filter(doesCardMatch).map((card) => (<SortableCard key={card.id} card={card} onClick={setActiveModal} getBgColor={getBgColor} variant="horizontal" />))}</div></SortableContext>
            </div>
          )}
          {missionSectionItems.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between group">
                <div className={`flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-wider pl-1 transition-colors ${canEdit ? "cursor-pointer hover:text-blue-500" : ""}`} onClick={() => canEdit && renameSection(missionsTitle, 'missions')}><i className="fas fa-plane-departure"></i> {missionsTitle}{canEdit && <i className="fas fa-pen opacity-0 group-hover:opacity-100 ml-2"></i>}</div>
              ...
              <SortableContext items={missionSectionItems} strategy={rectSortingStrategy} id="Missions"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 min-h-[100px] border-2 border-transparent hover:border-slate-200/50 border-dashed rounded-xl transition-all">{missionSectionItems.filter(doesCardMatch).map((card) => (<SortableCard key={card.id} card={card} onClick={setActiveModal} getBgColor={getBgColor} variant={card.id === 'missions-status' ? 'mission' : 'vertical'} />))}</div></SortableContext>
            </div>
          )}
          {sections.map((section) => {
            const sectionCards = [...manualCards, ...genericWidgets].filter(c => { const cat = c.settings?.category || "Planning & Resources"; return cat === section; }).filter(doesCardMatch);
            return (
              <div key={section} className="space-y-4 animate-in fade-in duration-500">
                <div className="flex items-center justify-between group">
                  <div className="flex items-center gap-2">
                    <div className={`flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-wider pl-1 transition-colors ${canEdit ? "cursor-pointer hover:text-blue-500" : ""}`} onClick={() => canEdit && renameSection(section, 'custom')}><i className="fas fa-layer-group"></i> {section}{canEdit && <i className="fas fa-pen opacity-0 group-hover:opacity-100 ml-2"></i>}</div>
                    {canEdit && (<button onClick={() => deleteSection(section)} className="text-slate-300 hover:text-red-500 transition-colors text-xs" title="Delete Section"><i className="fas fa-trash"></i></button>)}
                  </div>
                ...
                <SortableContext items={sectionCards} strategy={rectSortingStrategy} id={section}><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 min-h-[100px] rounded-xl border-2 border-dashed border-transparent p-2 transition-colors hover:border-slate-200/20">{sectionCards.map((card) => (<SortableCard key={card.id} card={card} onClick={setActiveModal} getBgColor={getBgColor} />))}{sectionCards.length === 0 && (<div className="col-span-full flex items-center justify-center text-slate-300 text-sm font-medium italic h-24">Drop cards here</div>)}</div></SortableContext>
              </div>
            );
          })}
          <DragOverlay>{activeDragItem ? (<div className={`rounded-2xl p-6 text-white flex flex-col items-center justify-center text-center h-40 shadow-2xl scale-105 ${getBgColor(activeDragItem.color || 'rose')}`}><h4 className="font-bold text-xl tracking-wide">{activeDragItem.title}</h4></div>) : null}</DragOverlay>
        </DndContext>
        <div className="pt-8 border-t border-slate-200 text-center">{canEdit && (<button onClick={addSection} className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-slate-100 text-slate-500 font-bold hover:bg-slate-200 transition-colors"><i className="fas fa-plus"></i> Add New Section</button>)}</div>
      </main>
      ...
      {activeCard && (
        <div onClick={() => setActiveModal(null)} className="fixed inset-0 z-50 flex items-center justify-center sm:px-4 sm:py-6 bg-slate-900/70 backdrop-blur-sm">
          {/* 🟢 MOBILE FIX: w-full h-full on mobile, rounded on desktop */}
          <div onClick={(e) => e.stopPropagation()} className={`bg-white w-full h-full sm:h-auto sm:rounded-2xl shadow-2xl relative overflow-hidden flex flex-col transition-all duration-300 ${showDocPreview || activeCard.type === 'generic-sheet' || activeCard.settings?.viewMode ? "sm:max-w-6xl sm:h-[85vh]" : "sm:max-w-2xl"}`}>
            ...
            {/* Header */}
            <div className={`${getBgColor(activeCard.color || 'rose')} p-4 md:p-6 flex justify-between items-center text-white shrink-0 transition-colors`}>
              <div>
                <h3 ref={titleRef} contentEditable={isEditing && canEdit} suppressContentEditableWarning={true} className={`text-xl md:text-2xl font-bold outline-none ${isEditing && canEdit ? 'border-b-2 border-white/50 bg-white/10 px-2 rounded cursor-text' : ''}`}>{activeCard.title}</h3>
                {isEditing && isCardEditable(activeCard) && canEdit && (<div className="flex gap-2 mt-3 animate-in fade-in slide-in-from-left-2 duration-200">{Object.keys(COLOR_MAP).map((c) => (<button key={c} onClick={(e) => { e.stopPropagation(); updateColor(c); }} className={`w-6 h-6 rounded-full border-2 border-white/40 hover:scale-110 transition-transform shadow-sm ${COLOR_MAP[c]} ${activeCard.color === c ? 'ring-2 ring-white scale-110' : ''}`} title={`Change to ${c}`} />))}</div>)}
              ...
              <div className="flex items-center gap-2 md:gap-3">
                ...
                {(activeCard.type === 'generic-sheet' || attachedSheetUrl || activeCard.data) && !isMapping && (
                  <button onClick={() => { if (activeCard.settings?.viewMode) { const updated = { ...activeCard, settings: { ...activeCard.settings, viewMode: null } }; setActiveCard(updated); } else { const targetMode = activeCard.settings?.chartType ? 'chart' : 'card'; const updated = { ...activeCard, settings: { ...activeCard.settings, viewMode: targetMode } }; setActiveCard(updated); if (attachedSheetUrl && !activeCard.data) { loadSheetData(attachedSheetUrl, updated); } } }} className={`px-3 py-1.5 rounded-lg text-xs md:text-sm font-bold transition-all flex items-center gap-2 shadow-sm border ${activeCard.settings?.viewMode ? "bg-slate-100 text-slate-700 border-slate-300 hover:bg-slate-200" : "bg-purple-600 text-white border-purple-500 hover:bg-purple-50 animate-pulse"}`}><i className={`fas ${activeCard.settings?.viewMode ? 'fa-folder-open' : 'fa-chart-pie'}`}></i><span className="hidden sm:inline">{activeCard.settings?.viewMode ? "Back to Files" : "Visualize Data"}</span></button>
                )}
                {activeCard.settings?.viewMode && (<button onClick={() => setIsMapping(true)} className={`px-3 py-1.5 rounded-lg text-sm font-bold bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 transition-all shadow-sm ${!canEdit ? 'hidden' : ''}`} title="Configure View"><i className="fas fa-sliders-h"></i></button>)}
                {(showDocPreview || activeCard.sheet_url || activeCard.source?.includes("sheet")) && (<a href={showDocPreview ? showDocPreview.replace("/preview", "/edit") : activeCard.sheet_url || activeCard.sheet_url_schedule} target="_blank" rel="noreferrer" className="px-3 py-1 rounded text-sm font-bold bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors border border-blue-200 flex items-center gap-2"><i className="fas fa-external-link-alt"></i> <span className="hidden sm:inline">Open in {activeCard.source?.includes("sheet") || activeCard.type === 'generic-sheet' ? "Sheets" : "Docs"}</span></a>)}
                {!showDocPreview && isCardEditable(activeCard) && canEdit && (<button onClick={toggleEditMode} className={`px-3 py-1 rounded text-sm font-medium transition-colors border ${isEditing ? 'bg-white text-slate-900 border-white' : 'bg-black/20 text-white border-transparent hover:bg-black/40'}`}><i className={`fas ${isEditing ? 'fa-check' : 'fa-pen'} mr-2`}></i><span className="hidden sm:inline">{isEditing ? "Done" : "Edit Card"}</span></button>)}
                {!showDocPreview && (isCardEditable(activeCard) || activeCard.type === 'generic-sheet') && canEdit && (<button onClick={() => deleteCard(activeCard)} className="bg-red-500 px-3 py-1 rounded text-sm font-bold hover:bg-red-600 transition-colors text-white" title="Delete Card"><i className="fas fa-trash"></i></button>)}
            ...
            <div className="p-0 overflow-y-auto custom-scroll flex flex-col h-full bg-slate-50">
              ...
              {activeCard.settings?.viewMode ? (
                ...
                (!activeCard.data && activeCard.type !== 'generic-sheet') ? (
                  <div className="flex h-full items-center justify-center text-slate-400 gap-3"><i className="fas fa-circle-notch fa-spin text-2xl text-blue-500"></i><span className="font-medium">Loading Spreadsheet Data...</span></div>
                ) : isMapping ? (
                  <div className="flex h-full flex-col md:flex-row">
                    {/* LEFT SIDEBAR: CONTROLS (Updated with text-slate-900 for dark mode support) */}
                    <div className="w-full md:w-1/3 bg-white text-slate-900 border-r border-slate-200 flex flex-col h-1/2 md:h-full">
                      <div className="p-6 border-b border-slate-100"><h2 className="text-lg font-bold text-slate-800 flex items-center gap-2"><i className="fas fa-magic text-purple-500"></i> Configure View</h2></div>
                      <div className="p-6 space-y-6 overflow-y-auto flex-1">
                        {/* View Mode Selector */}
                        <div>
                          <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-wider mb-2">Display Style</label>
                        ...
                        {activeCard.settings?.viewMode === 'chart' ? (
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <select value={activeCard.settings?.chartType || 'bar'} onChange={(e) => setActiveCard({ ...activeCard, settings: { ...activeCard.settings, chartType: e.target.value } })} className="w-full p-2.5 border border-slate-300 rounded-lg text-sm bg-white outline-none">
                            ...
                            {activeCard.settings?.chartType !== 'metric' && (
                              <div><label className="block text-xs font-bold text-slate-700 mb-1">X-Axis (Label)</label><select value={activeCard.settings?.xAxisCol || ''} onChange={(e) => setActiveCard({ ...activeCard, settings: { ...activeCard.settings, xAxisCol: e.target.value } })} className="w-full p-2.5 border border-slate-300 rounded-lg text-sm bg-slate-50 outline-none focus:ring-2 focus:ring-blue-500"><option value="">-- Select Column --</option>{activeCard.columns?.map((c: string) => <option key={c} value={c}>{c}</option>)}</select></div>
                            )}
                    ...
                    <div className="flex-1 bg-slate-100/50 flex flex-col h-1/2 md:h-full">
                        ...
                        {activeCard.settings?.viewMode === 'chart' ? (
                            ...
                            <div className="flex-1 min-h-0 flex flex-col justify-center">
                              {activeCard.settings?.chartType === 'metric' ? (
                                <div className="flex flex-col items-center gap-4">
                                  ...
                                  <div className="text-xs font-bold uppercase tracking-widest text-slate-500 bg-slate-100 px-3 py-1 rounded-full">{activeCard.settings.yAxisCol}</div>
                                </div>
                                    ...
                                    return (
                                      <div key={idx} className="w-full">
                                        ...
                                        <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                                          <div className="h-full rounded-full shadow-sm transition-all duration-500" style={{ width: `${percent}%`, backgroundColor: CHART_COLORS[idx % CHART_COLORS.length] }}></div>
                                        </div>
                                      </div>
                                    );
                                  })}
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                  {activeCard.settings?.chartType === 'pie' || activeCard.settings?.chartType === 'donut' ? (
                                    <PieChart>
                                      <Pie data={activeCard.data.map((d: any) => ({ name: d[activeCard.settings.xAxisCol], value: cleanNumber(d[activeCard.settings.yAxisCol]) }))} cx="50%" cy="50%" innerRadius={activeCard.settings?.chartType === 'donut' ? 60 : 0} outerRadius={80} paddingAngle={5} dataKey="value">
                          ...
                          <div className="w-full max-w-md pointer-events-none select-none bg-white p-6 rounded-xl border border-slate-200 shadow-xl scale-110 origin-center">
                            <div className="flex justify-between items-start mb-4">
                              <div><div className="text-[10px] font-bold text-slate-400 uppercase mb-1">{activeCard.settings?.titleCol || "Select Title"}</div><h4 className="font-bold text-xl text-slate-800 line-clamp-2">{activeCard.data[0]?.[activeCard.settings?.titleCol] || "Sample Value"}</h4></div>
                            </div>
                            <div className="space-y-3"><div className="h-2 bg-slate-100 rounded w-full"></div><div className="h-2 bg-slate-100 rounded w-2/3"></div></div>
                          </div>
                        ) : (
                          <div className="bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden w-full max-w-md pointer-events-none select-none">
                            <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex gap-4"><div className="h-2 bg-slate-200 rounded w-16"></div><div className="h-2 bg-slate-200 rounded w-16"></div></div>
                            <div className="p-4 space-y-4">{[1, 2, 3].map(i => (<div key={i} className="flex gap-4 items-center"><div className="h-3 bg-slate-800 rounded w-1/3 opacity-80"></div><div className="h-3 bg-slate-200 rounded w-1/4"></div></div>))}</div>
                          </div>
                        )}
                      </div>
                    </div>
                          ...
                          {activeCard.data.map((row: any, idx: number) => {
                            ...
                            return (
                              <div key={idx} className="w-full">
                                ...
                                <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                                  <div className="h-full rounded-full shadow-sm transition-all duration-1000 ease-out" style={{ width: `${percent}%`, backgroundColor: CHART_COLORS[idx % CHART_COLORS.length] }}></div>
                                </div>
                              </div>
                            );
                          })}
                  ...
                  <div className="p-4 md:p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {activeCard.data.filter((row: any) => row[activeCard.settings.titleCol]).map((row: any, idx: number) => (
                      <div key={idx} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
                        <div className="flex justify-between items-start mb-4">
                          <div><div className="text-[10px] font-bold text-slate-400 uppercase mb-1">{activeCard.settings.titleCol}</div><h4 className="font-bold text-xl text-slate-800 line-clamp-2">{row[activeCard.settings.titleCol] || "Untitled"}</h4></div>
                        ...
                        <div className="space-y-3">
                          {activeCard.settings.subtitleCol && (<div><div className="text-[10px] font-bold text-slate-400 uppercase">{activeCard.settings.subtitleCol}</div><div className="text-sm text-slate-600 font-medium">{row[activeCard.settings.subtitleCol]}</div></div>)}
                          {activeCard.settings.extraFields?.map((f: string) => (<div key={f}><div className="text-[10px] font-bold text-slate-400 uppercase">{f}</div><div className="text-sm text-slate-600 font-medium">{renderCellContent(row[f])}</div></div>))}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                !showDocPreview && (
                  <div className="p-0">
                    <div className="p-4 md:p-8">
                      {isEditing && canEdit && (<div className="flex flex-col gap-3">{getBlocks(activeCard).length === 0 && (<button onClick={() => updateResources([{ title: "General Files", items: [] }])} className="w-full py-4 border-2 border-dashed border-blue-200 bg-blue-50 text-blue-600 rounded-xl font-bold hover:bg-blue-100 hover:border-blue-400 transition-all shadow-sm touch-manipulation"><i className="fas fa-plus-circle mr-2"></i> Start Adding Files</button>)}<button onClick={addBlock} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 text-sm font-bold hover:border-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-all touch-manipulation">+ Create New Category Block</button></div>)}
                      {getBlocks(activeCard).map((block: any, bIdx: number) => (
                        <div key={bIdx} className="mb-8">
                          <div className="flex justify-between items-end mb-3 border-b border-slate-100 pb-1"><h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">{block.title}</h4>{isEditing && canEdit && <button onClick={() => deleteBlock(bIdx)} className="text-[10px] text-red-400 hover:text-red-600 uppercase font-bold touch-manipulation">Delete Block</button>}</div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{block.items.map((item: any, iIdx: number) => (<div key={iIdx} className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md hover:border-blue-400 transition-all group cursor-pointer relative touch-manipulation"><div className="flex items-center gap-4 flex-1" onClick={() => handleFileClick(item)}><div className="w-10 h-10 rounded-full bg-slate-50 text-slate-600 group-hover:bg-blue-50 group-hover:text-blue-500 flex items-center justify-center transition-colors">{getFileIcon(item)}</div><div className="font-bold text-slate-800 text-sm group-hover:text-blue-600 break-words">{item.title}</div></div>{isEditing && canEdit && <button onClick={() => deleteItemFromBlock(bIdx, iIdx)} className="absolute top-2 right-2 text-slate-200 hover:text-red-500 p-1 touch-manipulation"><i className="fas fa-times-circle"></i></button>}</div>))}</div>
                          {isEditing && canEdit && (
                            <div className="mt-4 pt-4 border-t border-slate-50">
                              {addingLinkToBlock === bIdx ? (<div className="flex flex-col md:flex-row items-center gap-2 w-full animate-in fade-in slide-in-from-left-2 duration-200"><input ref={(el) => { newItemTitleRefs.current[`block-${bIdx}`] = el }} type="text" placeholder="Name your file..." autoFocus className="w-full md:flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition-all" /><input ref={(el) => { newItemUrlRefs.current[`block-${bIdx}`] = el }} type="text" placeholder="Paste URL..." className="w-full md:flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:bg-white transition-all" /><div className="flex gap-2 w-full md:w-auto"><button onClick={() => addItemToBlock(bIdx)} className="flex-1 md:flex-none bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 transition-colors shadow-sm touch-manipulation">Add</button><button onClick={() => setAddingLinkToBlock(null)} className="flex-1 md:flex-none text-slate-400 hover:text-slate-600 px-2 transition-colors touch-manipulation"><i className="fas fa-times"></i></button></div></div>) : (<div className="flex flex-col sm:flex-row gap-3"><button onClick={() => handleOpenPicker(bIdx)} className="px-4 py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-colors bg-slate-100 text-slate-700 hover:bg-slate-200 touch-manipulation"><svg viewBox="0 0 87.3 78" width="14" height="14" className="opacity-80"><path d="M6.6 66.85l3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3l18.25-31.6H6.6v21.65zm13.1-22.8L6.6 19.85l-3.3 5.7C2.1 27.9 1.5 30.2 1.5 32.55v15.85l18.2-4.35zM22.65 6.6L19.35.9C18 .1 16.55-.3 15-.3h-3.4l18.25 31.65 14.2-24.6h-21.4zM62.6 6.6h-14.8l-7.1 12.35L54.1 36.3l14.8-25.6c-.65-1.9-1.9-3.35-3.65-4.1zM34.7 44.05l-7.1 12.3 10.75 18.6c2.35 0 4.7-.6 6.85-1.8L84.65 44.05H34.7zM66.45 44.05l18.2 31.6c1.35-.8 2.5-1.9 3.3-3.3l3.85-6.65-18.25-31.6-7.1 9.95z" fill="currentColor" /></svg>Add from Drive</button><button onClick={() => setAddingLinkToBlock(bIdx)} className="px-4 py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-colors bg-blue-50 text-blue-700 hover:bg-blue-100 touch-manipulation">+ Add Link</button></div>)}
                            </div>
                          )}
                        </div>
                      ))}
      ...
      {showTutorial && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 bg-slate-900/80 backdrop-blur-sm transition-opacity">
          {/* ... Tutorial Modal (No Changes) ... */}
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-slate-50 p-6 border-b border-slate-200 flex justify-between items-center"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-xl shadow-sm"><i className="fas fa-book-reader"></i></div><div><h3 className="text-xl font-bold text-slate-800">Quick Start Guide</h3><p className="text-slate-500 text-sm">Get the most out of your dashboard</p></div></div><button onClick={() => setShowTutorial(false)} className="w-8 h-8 flex items-center justify-center rounded-full text-slate-500 hover:text-slate-900 hover:bg-slate-200 transition-all" title="Close Guide"><i className="fas fa-times text-lg"></i></button></div>
            ...
            <div className="bg-slate-50 p-6 border-t border-slate-200 text-center"><button onClick={() => setShowTutorial(false)} className="bg-slate-800 hover:bg-slate-900 text-white font-bold py-3 px-10 rounded-xl transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 active:translate-y-0">Got it, let's go!</button></div>
          </div>
        </div>
      )}
